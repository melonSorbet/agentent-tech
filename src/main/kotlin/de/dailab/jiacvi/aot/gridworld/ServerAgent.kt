...
class ServerAgent(
	...
	override fun behaviour() = act {
		...
		on<TransferMaterial> { msg ->
			...
			if (!allowedToAct(msg.fromID) || !allowedToAct(msg.toID)) {println("super lolers") }
			...
			if (!colAgent.collector || repAgent.collector) {println("lolsen1")
				return@on }
			if (!colAgent.hasMaterial || repAgent.hasMaterial) {println("lolsen2")
				return@on }
